...
public class Navigation {
            ...
            final int maxThrust)
    {
        ...
        return navigateShipTowardsTarget(gameMap, ship, targetPos, maxThrust, avoidObstacles, maxCorrections, angularStepRad, 1);
    }
            ...
            
    {
        ...
        if (avoidObstacles && !gameMap.objectsBetween(ship, targetPos).isEmpty()) {
        	if(counter%2 == 0){
	            final double newTargetDx = Math.cos(angleRad + angularStepRad*counter) * distance;
	            final double newTargetDy = Math.sin(angleRad + angularStepRad*counter) * distance;
	            final Position newTarget = new Position(ship.getXPos() + newTargetDx, ship.getYPos() + newTargetDy);
            return navigateShipTowardsTarget(gameMap, ship, newTarget, maxThrust, true, (maxCorrections-1), angularStepRad, counter+1);
        	}
        	else{
	            final double newTargetDx = Math.cos(angleRad - angularStepRad*counter) * distance;
	            final double newTargetDy = Math.sin(angleRad - angularStepRad*counter) * distance;
